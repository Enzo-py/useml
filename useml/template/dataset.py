...
def load_dataset(dataset, config):
    """Returns ``(train_loader, val_loader)`` for a dataset specification.

    Args:
        dataset: One of:
            - A string key: ``"mnist"``, ``"fashion_mnist"``, ``"cifar10"``,
              ``"cifar100"``.
            - A HuggingFace dataset name prefixed with ``"hf:"``.
            - Any ``torch.utils.data.Dataset`` — split automatically.
        config: A :class:`~useml.Config` instance supplying ``batch_size``,
            ``val_split``, ``seed``, ``num_workers``, and ``data_dir``.

    Returns:
        A ``(train_loader, val_loader)`` tuple of DataLoader instances.

    Raises:
        ValueError: If *dataset* is an unknown string key.
        TypeError: If *dataset* is neither a string nor a Dataset.
    """
    if isinstance(dataset, str):
        ...
        raise ValueError(
            ...
            f"Built-in options: {list(_BUILTIN)}. "
            "For HuggingFace use 'hf:<dataset_name>'."
        )
    if hasattr(dataset, "__len__"):
        return _wrap_custom(dataset, config)
    raise TypeError(
        f"dataset must be a string or a torch Dataset, "
...
def _load_huggingface(hf_name: str, config):
    ...
    train_full = _HFWrapper(raw[split_name])
    train_ds, val_ds = _split(train_full, config.val_split, config.seed)
    return _make_loader(train_ds, config, shuffle=True), \
           _make_loader(val_ds, config, shuffle=False)
